...
void Move(char Field[], const int n, char Player)
{
	...
	/*do
	{
		...
		if (busy)cout << " Клетка занята" << endl;
	} while (busy);*/
	do
	{
		busy = false;
...
void Check(char Field[], const int n, char Player)
{
	bool game_over = false;
	 if (Field[0] == Field[4] && Field[4] == Field[8] && Field[0] != 0) game_over = true; //по диагонали
else if (Field[2] == Field[4] && Field[4] == Field[6] && Field[2] != 0) game_over = true;

else if (Field[0] == Field[1] && Field[1] == Field[2] && Field[2] != 0) game_over = true; // по горизонтали 
...
else if (Field[0] == Field[3] && Field[3] == Field[6] && Field[6] != 0) game_over = true;// по вертикали 
